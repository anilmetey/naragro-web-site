...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 9. Pure White Executive Splash Screen
    setupSplashScreen();
});
...
/**
 * Pure White Executive Brand Curtain (Quiet Luxury)
 */
function setupSplashScreen() {
    const splash = document.getElementById('splashScreen');
    if (!splash) return;

    const progressFill = document.getElementById('whiteProgressFill');
    ...
    function dismissSplash() {
        ...
        splash.classList.add('white-splash-exit');
    ...
    // Smooth line progress over ~1.6 seconds
    const duration = 1600;
    const interval = 25;
    ...
    const progressTimer = setInterval(() => {
        ...
        const pct = Math.min(100, Math.round((1 - Math.pow(1 - t, 2.2)) * 100));
